...
class TestUserRegistration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data = UserRegistration('firstname', 'lastname', 'othername', 'phoneNumber', 'username', 'email', 'password', 'confirm_password', 'isAdmin')
    ...
    def test_user_login(self):
        ...
        response_msg = json.loads(response.data.decode('UTF-8'))
        self.assertIn("Registration Successful", response_msg["RegistrationMessage"])
        response = self.client.post("/api/v1/auth/login", data=json.dumps(self.logger), content_type="application/json")
        self.assertEqual(response.status_code, 200)
        response_msg = json.loads(response.data.decode('UTF-8'))
        self.assertIn("Login Successful", response_msg["LoginMessage"])

    def test_user_login_blank_username(self):
        ''' tests that a user cannot log in with blank username '''
        response = self.client.post("/api/v1/auth/signup", data=json.dumps(self.user), content_type="application/json")
        self.assertEqual(response.status_code, 201)
        response_msg = json.loads(response.data.decode('UTF-8'))
        self.assertIn("Registration Successful", response_msg["RegistrationMessage"])
        response2 = self.client.post("/api/v1/auth/login", data=json.dumps(dict(username="", password="thisispass")), content_type="application/json")
    ...
    def test_user_login_blank_password(self):
        ...
        response_msg = json.loads(response.data.decode('UTF-8'))
        self.assertIn("Registration Successful", response_msg["RegistrationMessage"])
        response2 = self.client.post("/api/v1/auth/login", data=json.dumps(dict(username="dennisdnyce", password="")), content_type="application/json")
        self.assertEqual(response2.status_code, 401)

    def test_user_login_mismatch_credentials(self):
        ''' tests that a user cannot log in with incorrect username or password '''
        response = self.client.post("/api/v1/auth/signup", data=json.dumps(self.user), content_type="application/json")
        self.assertEqual(response.status_code, 201)
        response_msg = json.loads(response.data.decode('UTF-8'))
        self.assertIn("Registration Successful", response_msg["RegistrationMessage"])
        response2 = self.client.post("/api/v1/auth/login", data=json.dumps(dict(username="dennis", password="passthis")), content_type="application/json")
